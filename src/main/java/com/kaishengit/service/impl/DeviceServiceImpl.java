...
public class DeviceServiceImpl implements DeviceService {
    ...
    public String saveRent(DeviceRentDto deviceRentDto) {
        //1. 保存合同，多个事务，加事务管理@Transactional
    ...
    public InputStream downloadFileById(Integer id) throws IOException {
        ...
        if(rentDoc == null) {
            ...
            File file = new File(fileSavePath + "/" + rentDoc.getNewName());
            if(file.exists()) {
    ...
    public void downloadZipFile(DeviceRent rent, ZipOutputStream zipOutputStream) throws IOException {
        //查找合同有多少个文件
        List<DeviceRentDoc> rentDocList = findDeviceRentDocListByRentId(rent.getId());
        for(DeviceRentDoc doc : rentDocList) {
            //压缩文件展开每一项都是一个entry
            ZipEntry entry = new ZipEntry(doc.getSourceName());
            zipOutputStream.putNextEntry(entry);

            InputStream inputStream = downloadFileById(doc.getId());
    ...
    @Override
    public List<DeviceRent> findDeviceRentListByQueryParam(Map<String, Object> queryParam) {
        return rentMapper.findByQueryParam(queryParam);
    }

    @Override
    public Long countOfDeviceRent() {
        return rentMapper.countOfDeviceRent();
    }
